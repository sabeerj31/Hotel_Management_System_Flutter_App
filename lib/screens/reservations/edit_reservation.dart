...
class _EditReservationScreenState extends State<EditReservationScreen> {
  ...
  void initState() {
    super.initState();
    _guestNameController.text = widget.reservation.guestName;
    _guestEmailController.text = widget.reservation.guestEmail;
    _guestPhoneController.text = widget.reservation.guestPhone;
    _selectedRoomType = widget.reservation.roomType;
    ...
    _guestsController.text = widget.reservation.numberOfGuests.toString();
    _totalAmountController.text = widget.reservation.totalAmount.toStringAsFixed(2);

    _checkInController.text = DateFormat('yyyy-MM-dd').format(widget.reservation.checkInDate);
    _checkOutController.text = DateFormat('yyyy-MM-dd').format(widget.reservation.checkOutDate);
  }
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              SwitchListTile(
                title: const Text('Confirmed Status'),
                value: _status,
                ...
                secondary: Icon(_status ? Icons.check_circle : Icons.pending,
                    color: _status ? Colors.green : Colors.orange),
              ),
              const SizedBox(height: 16),
  ...
  Future<void> _selectCheckOutDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: _checkOutDate ?? (_checkInDate ?? DateTime.now()).add(const Duration(days: 1)),
  ...
  void _submitForm() async {
    ...
    if (_formKey.currentState!.validate() &&
      ...
      try {
        ...
        await provider.updateReservation(updatedReservation);
